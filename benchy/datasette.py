...
def download_and_open_datasette(
...
def fetch_datasette(
    ...
    df = download_and_open_datasette(
        url + ".csv?_dl=on&_stream=on&_size=max",
        name,
        data_home,
        force,
        cleanup=cleanup,
...
def fetch_births_multistate(return_X_y=False, data_home=None, force=False):
    def cleanup(dataf):
        ...
        return dataf.with_columns(
            date=expr.cast(pl.Date), state=pl.col("state").cast(pl.Categorical)
        ).select("state", "date", "births")
...
def fetch_bigmac(return_X_y=False, data_home=None, force=False):
    def cleanup(dataf):
        return dataf.with_columns(
            ...
            country=pl.col("name").cast(pl.Categorical),
        ).select("date", "country", "dollar_price")
...
def fetch_smoking(return_X_y=False, data_home=None, force=False):
    def cleanup(dataf):
        return dataf.with_columns(
            outcome=pl.col("outcome").cast(pl.Categorical),
            smoker=pl.col("smoker").cast(pl.Categorical),
        ).select("outcome", "smoker", "age")
